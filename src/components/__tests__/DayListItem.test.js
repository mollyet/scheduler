...
it("renders 'Sorry, no spots remaining!' when there are 0 spots", () => {
  const { getByText } = render(<DayListItem name="Monday" spots={0} />);
  expect(getByText("Sorry, no spots remaining!")).toBeInTheDocument();
});

it("renders '1 spot remaining!' when there is 1 spot", () => {
  const { getByText } = render(<DayListItem name="Monday" spots={1} />);
  expect(getByText("One spot remaining!")).toBeInTheDocument();
});
